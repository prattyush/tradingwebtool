...
const TradeReplayer = () => {
    ...
    const handleSellOptions = (event) => {
        ...
        //}
        performSellAction(sellPrice, optionType, optionNextOneMinBarData['time']-1)
    }
    ...
    const handleTimeNext = (event) => {
        ...
        if (currentTotalPEQuantity.current > 0 || currentTotalCEQuantity.current > 0) {
            ...
            } else if (currentTotalPEQuantity.current > 0 && peData['low'] <= manageTarget+0.15 && peData['high'] >= manageTarget+0.15) {
                performSellAction(manageTarget-0.15, "Put", peData['time']-1)
            } else if (currentTotalCEQuantity.current > 0 && ceData['low'] <= manageTarget+0.15 && ceData['high'] >= manageTarget+0.15) {
                performSellAction(manageTarget-0.15, "Call", ceData['time']-1)
            }
        }
